...
export function DuvidasClient({ categorias }: { categorias?: typeof faqCategories }) {
  ...
  return (
    <>
        ...
        <div className="container">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="relative max-w-xl mx-auto">
              ...
              <Input placeholder="Buscar dúvidas..." className="pl-12 py-6 text-lg bg-white" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Category Filter */}
      <section className="py-6 bg-[#F6F6F6] border-b border-[#D6D6D6] sticky top-[102px] md:top-[116px] z-30">
        <div className="container">
          ...
          <div className="max-w-3xl mx-auto">
            {filteredCategories.length === 0 && (
              <div className="text-center py-12">
                ...
                <button onClick={() => { setSearchTerm(""); setActiveCategory(null); }} className="mt-4 text-[#001A9E] hover:underline">Limpar busca</button>
              </div>
            )}
            {filteredCategories.map((category) => (
              <div key={category.name} className="mb-12">
                <div className="flex items-center gap-3 mb-6">
                  <h2 className="text-2xl font-bold text-[#2D2D2D]">{category.name}</h2>
                  ...
                  {category.faqs.map((faq, index) => {
                    ...
                    return (
                      <div key={faqId} className="border border-[#D6D6D6] rounded-xl overflow-hidden">
                        <button className="w-full p-5 flex items-center justify-between text-left hover:bg-[#F6F6F6] transition-colors" onClick={() => toggleFaq(faqId)}>
                          <span className="font-medium text-[#2D2D2D] pr-4">{faq.question}</span>
                          {openFaq === faqId ? <ChevronUp className="w-5 h-5 flex-shrink-0 text-[#FF9100]" /> : <ChevronDown className="w-5 h-5 flex-shrink-0 text-[#999999]" />}
                        </button>
                        {openFaq === faqId && (
                          <div className="px-5 pb-5 text-[#555555] border-t border-[#E8E8E8] pt-4 whitespace-pre-line leading-relaxed">{faq.answer}</div>
                        )}
                      </div>
                    );
                  })}
        ...
        <AnimateOnScroll variant="zoom-in">
          <div className="container">
            <div className="max-w-2xl mx-auto text-center">
              ...
              <h2 className="text-2xl font-bold text-[#2D2D2D] mb-4">Não encontrou o que procurava?</h2>
              <p className="text-[#555555] mb-6">Nossa <strong>equipe está pronta</strong> para ajudar você com qualquer dúvida.</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link href="/contato"><Button className="bg-[#FF9100] hover:bg-[#E68200]">Falar com atendente</Button></Link>
                <a href="https://api.whatsapp.com/send?phone=5508007770808" target="_blank" rel="noopener noreferrer"><Button variant="outline" className="border-[#001A9E] text-[#001A9E]">WhatsApp</Button></a>
              </div>
            </div>
          </div>
        </AnimateOnScroll>
